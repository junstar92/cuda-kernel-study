...
struct KernelTraits {
  ...
  static constexpr int kNumElemsSmemA = kThreadblockK * kSmemStrideA;
  static constexpr int kSmemStrideB = kThreadblockN + kSmemPaddingB;
  static constexpr int kNumElemsSmemB = kThreadblockK * kSmemStrideB;
  static constexpr int kTotalSmemElems = kNumElemsSmemA + kNumElemsSmemB;
  ...
  static constexpr int kThreadblockALoads =
      (kThreadblockM * kThreadblockK) / (kNumThreads * kVecWidth);
  static constexpr int kThreadblockBLoads =
      (kThreadblockK * kThreadblockN) / (kNumThreads * kVecWidth);
  ...
  static_assert(kThreadblockK % kVecWidth == 0,
                "BLOCK_K must be divisible by 4.");
  static_assert(kThreadblockN % kVecWidth == 0,
                "BLOCK_N must be divisible by 4.");
  static_assert((kThreadblockM * kThreadblockK) % (kNumThreads * kVecWidth) ==
                    0,
                "A tile must be evenly covered by vec4 accesses.");
  static_assert((kThreadblockK * kThreadblockN) % (kNumThreads * kVecWidth) ==
                    0,
                "B tile must be evenly covered by vec4 accesses.");
};
...
__device__ __forceinline__ void load_a_thread_fragment(
    float4 (&tb_frag_a)[Traits::kThreadblockALoads], float const* a_ptr,
    int lda, int tile_k) {
  constexpr int kPacketsPerRow = Traits::kThreadblockK / Traits::kVecWidth;

#pragma unroll
  for (int iter = 0; iter < Traits::kThreadblockALoads; ++iter) {
    int const packet_idx = threadIdx.x + iter * Traits::kNumThreads;
    int const row = packet_idx / kPacketsPerRow;
    int const col = (packet_idx % kPacketsPerRow) * Traits::kVecWidth;

    tb_frag_a[iter] = utils::load_float4(a_ptr + row * lda + tile_k + col);
  }
}

template <typename Traits>
__device__ __forceinline__ void load_b_thread_fragment(
    float4 (&tb_frag_b)[Traits::kThreadblockBLoads], float const* b_ptr,
    int ldb, int tile_k) {
  constexpr int kPacketsPerRow = Traits::kThreadblockN / Traits::kVecWidth;
...
#pragma unroll
  for (int iter = 0; iter < Traits::kThreadblockBLoads; ++iter) {
    ...
    tb_frag_b[iter] = utils::load_float4(b_ptr + (tile_k + row) * ldb + col);
  }
}

template <typename Traits>
__device__ __forceinline__ void store_a_thread_fragment_to_smem(
    float* smem_a, float4 const (&tb_frag_a)[Traits::kThreadblockALoads]) {
  constexpr int kPacketsPerRow = Traits::kThreadblockK / Traits::kVecWidth;
...
#pragma unroll
  for (int iter = 0; iter < Traits::kThreadblockALoads; ++iter) {
    ...
    float4 const value = tb_frag_a[iter];
    smem_a[(col + 0) * Traits::kSmemStrideA + row] = value.x;
    smem_a[(col + 1) * Traits::kSmemStrideA + row] = value.y;
    smem_a[(col + 2) * Traits::kSmemStrideA + row] = value.z;
    smem_a[(col + 3) * Traits::kSmemStrideA + row] = value.w;
  }
}

template <typename Traits>
__device__ __forceinline__ void store_b_thread_fragment_to_smem(
    float* smem_b, float4 const (&tb_frag_b)[Traits::kThreadblockBLoads]) {
  constexpr int kPacketsPerRow = Traits::kThreadblockN / Traits::kVecWidth;
...
#pragma unroll
  for (int iter = 0; iter < Traits::kThreadblockBLoads; ++iter) {
    ...
    utils::store_float4(smem_b + row * Traits::kSmemStrideB + col,
                        tb_frag_b[iter]);
  }
}
...
#pragma unroll
  for (int k = 0; k < Traits::kThreadblockK; ++k) {
    float4 const a_vec_0 = utils::load_float4(
        warp_a_ptr + k * Traits::kSmemStrideA + thread_m * Traits::kVecWidth);
    float4 const a_vec_1 = utils::load_float4(
        warp_a_ptr + k * Traits::kSmemStrideA +
        (thread_m + Traits::kWarpThreadsM) * Traits::kVecWidth);
    float4 const b_vec_0 = utils::load_float4(
        warp_b_ptr + k * Traits::kSmemStrideB + thread_n * Traits::kVecWidth);
    float4 const b_vec_1 = utils::load_float4(
        warp_b_ptr + k * Traits::kSmemStrideB +
        (thread_n + Traits::kWarpThreadsN) * Traits::kVecWidth);

    float const a[8] = {a_vec_0.x, a_vec_0.y, a_vec_0.z, a_vec_0.w,
                        a_vec_1.x, a_vec_1.y, a_vec_1.z, a_vec_1.w};
    float const b[8] = {b_vec_0.x, b_vec_0.y, b_vec_0.z, b_vec_0.w,
                        b_vec_1.x, b_vec_1.y, b_vec_1.z, b_vec_1.w};
...
#pragma unroll
      for (int m_pair = 0; m_pair < 4; ++m_pair) {
        ...
        accum[(m + 0) * 8 + (n + 0)] =
            fmaf(a[m + 0], b[n + 0], accum[(m + 0) * 8 + (n + 0)]);
        accum[(m + 1) * 8 + (n + 0)] =
            fmaf(a[m + 1], b[n + 0], accum[(m + 1) * 8 + (n + 0)]);
        accum[(m + 1) * 8 + (n + 1)] =
            fmaf(a[m + 1], b[n + 1], accum[(m + 1) * 8 + (n + 1)]);
        accum[(m + 0) * 8 + (n + 1)] =
            fmaf(a[m + 0], b[n + 1], accum[(m + 0) * 8 + (n + 1)]);
      }
    }
  }
}
...
#pragma unroll
      for (int m = 0; m < Traits::kLaneMmaShape; ++m) {
        ...
        float* dst =
            c_ptr + (row_group_base + m) * ldc + col_group_base;

        utils::store_float4(
            dst, make_float4(accum[accum_row * Traits::kThreadTileN +
                                   col_group * Traits::kLaneMmaShape + 0],
                             accum[accum_row * Traits::kThreadTileN +
                                   col_group * Traits::kLaneMmaShape + 1],
                             accum[accum_row * Traits::kThreadTileN +
                                   col_group * Traits::kLaneMmaShape + 2],
                             accum[accum_row * Traits::kThreadTileN +
                                   col_group * Traits::kLaneMmaShape + 3]));
      }
    }
  }
}
...
template <typename Traits>
__global__ __launch_bounds__(Traits::kNumThreads) void sgemm_warptiling_vec4(
  ...
  float* smem_a = smem;
  float* smem_b = smem + Traits::kNumElemsSmemA;

  float accum[64] = {0.0f};
  float4 tb_frag_a[Traits::kThreadblockALoads];
  float4 tb_frag_b[Traits::kThreadblockBLoads];
  ...
  load_a_thread_fragment<Traits>(tb_frag_a, a_ptr, K, 0);
  load_b_thread_fragment<Traits>(tb_frag_b, b_ptr, N, 0);
