...
// - Each thread computes THREAD_M elements along the M dimension
...
__global__ void sgemm_1d_blocktiling_kernel(float* __restrict__ c_ptr,
...
#pragma unroll
    for (int k = 0; k < BLOCK_K; k++) {
      float b_val = b_smem[k * BLOCK_N + thread_col];
...
#pragma unroll
  for (int m = 0; m < THREAD_M; m++) {
    c_ptr[(thread_row * THREAD_M + m) * N + thread_col] = accum[m];
  }
}
...
void launch_sgemm_1d_blocktiling(float* c_ptr, float const* a_ptr,
  ...
  sgemm_1d_blocktiling_kernel<BLOCK_M, BLOCK_N, BLOCK_K, THREAD_M,
                              A_SMEM_PADDING>
      <<<grid, block, SMEM_SIZE>>>(c_ptr, a_ptr, b_ptr, M, N, K);
}

template void launch_sgemm_1d_blocktiling<64, 64, 8, 8>(float*, float const*,
                                                        float const*, int const,
                                                        int const, int const);
template void launch_sgemm_1d_blocktiling<128, 128, 8, 64>(float*, float const*,
                                                           float const*,
                                                           int const, int const,
                                                           int const);