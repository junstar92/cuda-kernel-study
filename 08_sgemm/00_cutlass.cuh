...
template <int BLOCK_M, int BLOCK_N, int BLOCK_K, int WARP_M, int WARP_N,
          int WARP_K, int STAGES, int ThreadblockSwizzleN = 1>
struct KernelTraits {
  using Element = float;
...
template <typename Traits>
inline void launch_universal_with_source(float* d_ptr, float const* c_ptr,
                                         float const* a_ptr, float const* b_ptr,
                                         int const M, int const N, int const K,
                                         float const alpha = 1.0f,
                                         float const beta = 0.0f) {
  using Gemm = typename Traits::GemmUniversal;

  typename Gemm::Arguments arguments{
      ::cutlass::gemm::GemmUniversalMode::kGemm,
      {M, N, K},
      1,
      typename Traits::EpilogueOp::Params{alpha, beta},
      a_ptr,
      b_ptr,
      c_ptr,
      d_ptr,
  ...
  run_gemm<Gemm>(arguments);
}

template <int BLOCK_M, int BLOCK_N, int BLOCK_K, int WARP_M, int WARP_N,
          int WARP_K, int STAGES, int ThreadblockSwizzleN = 1>
inline void launch_universal(float* c_ptr, float const* a_ptr,
                             float const* b_ptr, int const M, int const N,
                             int const K) {
  using Traits = KernelTraits<BLOCK_M, BLOCK_N, BLOCK_K, WARP_M, WARP_N, WARP_K,
                              STAGES, ThreadblockSwizzleN>;
  launch_universal_with_source<Traits>(c_ptr, c_ptr, a_ptr, b_ptr, M, N, K,
                                       1.0f, 0.0f);
}
