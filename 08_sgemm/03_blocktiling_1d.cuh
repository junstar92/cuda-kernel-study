 ...
 * - each thread computes THREAD_M results along the M dimension
...
template <int BLOCK_M, int BLOCK_N, int BLOCK_K, int THREAD_M,
          int SMEM_A_PADDING>
__global__ void sgemm_blocktiling_1d(float* __restrict__ c_ptr,
                                     float const* __restrict__ a_ptr,
                                     float const* __restrict__ b_ptr,
                                     int const M, int const N, int const K) {
  constexpr int num_threads = (BLOCK_M * BLOCK_N) / THREAD_M;
  ...
  float accum[THREAD_M] = {0.f};

  // thread mapping inside the block tile
  int const tile_row = threadIdx.x / BLOCK_N;
  int const tile_col = threadIdx.x % BLOCK_N;
...
#pragma unroll
    for (int k = 0; k < BLOCK_K; ++k) {
      float const elem_b = smem_b[BLOCK_N * k + tile_col];

#pragma unroll
      for (int thread_m_idx = 0; thread_m_idx < THREAD_M; ++thread_m_idx) {
        float const elem_a =
            smem_a[smem_stride_a * k + THREAD_M * tile_row + thread_m_idx];
...
#pragma unroll
  for (int m = 0; m < THREAD_M; ++m) {
    c_ptr[(THREAD_M * tile_row + m) * N + tile_col] = accum[m];
  }
}
...
inline void launch(float* c_ptr, float const* a_ptr, float const* b_ptr,
  ...
  sgemm_blocktiling_1d<BLOCK_M, BLOCK_N, BLOCK_K, THREAD_M, SMEM_A_PADDING>
