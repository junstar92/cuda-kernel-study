...
#pragma unroll
    for (int warp_mma_k = 0; warp_mma_k < Traits::kWarpGemmIterations;
      ...
      v7::vec4::warp_mma<Traits>(accum, warp_frag_a[warp_mma_k % 2],
  ...
  v7::vec4::epilogue<Traits>(c_ptr, accum, smem, warp_tile_m_idx,
                             warp_tile_n_idx, lane_m_idx, lane_n_idx, tid, N);
}
...
inline void launch(float* c_ptr, float const* a_ptr, float const* b_ptr,
  ...
  using Traits = v7::vec4::KernelTraits<ThreadblockM, ThreadblockN,
                                        ThreadblockK, WarpM, WarpN, WarpK, 2>;
