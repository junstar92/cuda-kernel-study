...
## 12. Next Steps: Threadblock Swizzling, `cp.async`, Multistage, and Tensor Core
...
- Key idea: change the order in which CTAs visit output tiles so that inter-CTA cache reuse improves, especially at the L2 level.
- Why it matters: once the single-CTA `LDG -> STS -> LDS -> FFMA` path is already reasonably clean, CTA ordering alone can still move L2 hit rate and DRAM traffic by a meaningful amount.
- When to consider it: when the epilogue and 2-stage pipeline are already stable, but load-side L2 residency still trails a strong reference kernel.
- What to expect: threadblock swizzling does not change the instruction class of the kernel. It mainly changes launch order, so the biggest effects usually show up in cache-residency and DRAM-bandwidth metrics rather than seam-local dependency metrics.
- Common misunderstanding: swizzling does not make the mainloop itself more CUTLASS-like. It changes inter-CTA reuse, not the inner CTA seam.
...
13. If needed, adjust inter-CTA cache reuse with threadblock swizzling.
14. Then move on to `cp.async`, multistage, or Tensor Core only if needed.
